...
# Definir una función para actualizar el gráfico
def update(frame):
    if frame >= 3:
        y_index = frame - 3
        if y_index < len(y_t):
            line3.set_data(ty[: y_index + 1], y_t[: y_index + 1])
        return (line3,)


# Crear una figura y un único subplot para la animación
fig, ax3 = plt.subplots(1, 1)

# Crear un gráfico inicial vacío para la convolución
(line3,) = ax3.plot([], [], lw=2, label="Convolución")

# Establecer los límites y etiquetas del gráfico
ax3.set_xlim(min(ty), max(ty))
ax3.set_ylim(min(y_t), max(y_t) + 0.1 * max(y_t))
ax3.set_title("y[n]")
ax3.legend(loc="upper right")

# Crear la animación
num_frames = len(y_t) + 3
frame_rate = 10
ani = FuncAnimation(
    fig,
    update,
